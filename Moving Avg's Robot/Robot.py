...
print(f"""
Current Buying Power Available: {account.buying_power}""")
user_input = input("""
Use buying power (bp) or share count (qty) to trade? :""")
if user_input == 'bp':
    ...
    sys.exit("""
Cannot understand input. Try again.""")
...
fast_MA = input("""
Enter selected Fast Moving Average :""")
...
while Portfolio.is_market_open():
    ...
    Data.print_signal_update(data, current_signal, fast_MA, slow_MA)
    # Executing trade
    total_trades = Trading.execute_trade(data, current_signal, trade_limit, total_trades, ticker, buy_qty, fast_MA, slow_MA)
    # # If trade filled, add it to the total count
    # if filled_status == True:
